...
| 功能 | 说明 |
| --- | --- |
| 🕒 时间维度 | 每时段（每次工作）、每日、每月（**工作月**：每月 20 日到下月 19 日）、近 1 / 3 / 7 / 15 / 30 / 60 / 90 日 |
| 📊 图表 | 每日收入曲线、工作月柱状图、近 12 周热力图、时段分布环形图、星期分布条形图、KPI 卡片（含趋势与环比） |
| 📝 记录 | 填写日期、开始或结束时间、耗时（小时）、单数即可；收入按 `单数 × 单价` 自动计算 |
| 🌐 中英切换 | 顶栏「中 / EN」一键切换 |
| 💱 货币切换 | 顶栏「¥ / $」切换人民币 / 美元显示 |
| 🌙 主题 | 顶栏按钮循环切换 浅色 → 深色 → 跟随系统 |
| 💰 结算 | 顶部实时显示当前工作月、结算日（每月 20 日）与倒计时 |
...
3. **单价调整**：如果单价变化，在设置里修改「每单收入」即可，历史记录会自动按新单价重算。
4. **汇率**：美元显示使用设置中的汇率（默认 1 USD = 6.75 CNY），可根据汇率自行修改。

## 数据说明

- 数据通过 `localStorage` 保存在本地，刷新 / 关闭页面不会丢失。
- `/demo` 使用内置的模拟数据（仅用于体验），不会写入你的真实记录。
- 工作月定义：每月 **20 日** 起算到下月 **19 日**，20 日结算工资。
