...
| 📚 多账本 | 点击右上角账本按钮打开右侧边栏，可新建、重命名、删除多个账本，每个账本可设置独立的「每单收入」；勾选多个账本即可预览合并后的数据 |
| 🌐 中英切换 | 顶栏「中 / EN」一键切换 |
| 💱 货币切换 | 顶栏「¥ / $」切换人民币 / 美元显示 |
...
4. **单价调整**：单价按账本分别设置。点击右上角的账本按钮打开右侧边栏，点击某个账本的 ✎ 可修改它的「每单收入」，历史记录会自动按该账本的新单价重算；勾选多个账本时，合并数据会按各自单价正确汇总。
...
- 多账本：数据按账本组织（存储键 `wp.ledgers.v1`），旧的单账本数据会自动迁移到「默认账本」；每个账本可单独设置单价。
